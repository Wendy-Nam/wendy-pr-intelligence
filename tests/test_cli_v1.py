...
from prmonitor.__main__ import build_parser, main


def test_public_help_lists_only_product_commands():
    help_text = build_parser().format_help()

    for command in ("market-brief", "self-brief", "self-brief-daily", "init", "doctor"):
        assert command in help_text
    for command in ("memory-promote", "repair", "resume", "runs"):
...
def test_runs_lists_created_run(capsys, tmp_path):
    ...
    assert [record["run_id"] for record in json.loads(capsys.readouterr().out)["runs"]] == [run_id]


def test_unknown_run_id_is_structured_error(capsys, tmp_path):
    _run_id, state_dir = _prepared(tmp_path, capsys)
