"""PR 클리핑 일일 일괄 실행 (CLI: pr-clip-daily) — Python port of scripts/pr/run-pr-daily.sh.

Routines 진입점. 전처리(pre) → PR 클리핑(pr_clip) 순서를 보장하는 얇은
...
Ported from run-pr-daily.sh:
  ...
  L23  run-pr-monitor.sh "$DATE"            → pr_clip.run (date 만; hours 재해석)

run-pr-monitor.sh 는 HOURS 를 인자로 받지 않고 내부에서 다시
resolve_hours pr_monitoring 으로 해석한다(L24-28). 동일 정책이므로 결과는
같다 — 여기서도 pre 에만 hours 를 넘기고 pr_clip 에는 date 만 넘긴다.
"""
...
def run(args) -> int:
    ...
    # function) so importing this module never hard-depends on pre/pr_clip.
    from . import pre, pr_clip
    ...
    # 같은 hours 를 pr_clip 에도 넘긴다(override 일관성). 인자 없으면 pr_clip 이
    # 동일 정책으로 재해석하므로 결과는 같다.
    return pr_clip.run(Namespace(date=date, hours=hours))
